...
public sealed class CatVodHttpClient
{
 ...
 public async Task<JsonDocument> GetDetailAsync(string api, string id, CancellationToken cancellationToken = default)
 { var separator = api.Contains('?') ? "&" : "?"; return await GetJsonAsync($"{api}{separator}ac=detail&ids={Uri.EscapeDataString(id)}", cancellationToken); }
