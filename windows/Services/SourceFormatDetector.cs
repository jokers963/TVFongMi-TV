...
public static class SourceFormatDetector
{
    public static SourceFormat Detect(string content, string? fileName = null)
    {
        ...
        try
        {
            ...
            if (root.ValueKind == JsonValueKind.Array) return SourceFormat.SourceList;
            if (root.ValueKind == JsonValueKind.Object && (root.TryGetProperty("sites", out _) || root.TryGetProperty("urls", out _))) return SourceFormat.CatVodConfig;
        }
        catch (JsonException) { }
