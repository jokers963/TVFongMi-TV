...
{
    private readonly SourceCatalog _catalog = new(); private readonly SourceImporter _importer = new(); private readonly SourceHealthChecker _healthChecker = new();
    ...
    private async void CheckSources_Click(object sender, RoutedEventArgs e) { if (_catalog.Sources.Count == 0) { MessageBox.Show("请先导入影视源。", "提示"); return; } try { var results = await _healthChecker.CheckAllAsync(_catalog.Sources); var ok = results.Count(x => x.IsReachable); MessageBox.Show($"可访问 {ok}/{results.Count} 个影视源。", "检查完成"); } catch (Exception ex) { MessageBox.Show(ex.Message, "检查失败"); } }
